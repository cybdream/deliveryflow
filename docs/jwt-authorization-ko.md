...
배송 기사는 본인에게 배정된 배송만 상태 변경 또는 이력 조회를 할 수 있으며, `GET /api/v1/deliveries/me`으로 자신의 배송 목록을 조회합니다. 기사가 변경할 수 있는 상태는 `IN_DELIVERY`, `DELIVERED`, `ON_HOLD`입니다. `ASSIGNED` 복귀와 `CANCELLED`는 관리자만 처리합니다. 변경 이력의 처리자 이름은 요청 본문이 아니라 JWT의 로그인 이메일로 자동 기록됩니다.
